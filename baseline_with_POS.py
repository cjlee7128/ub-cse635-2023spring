...
def build_data(data_path, hal_data_path): 
    ...
    question_list = ['Is the response hallucinated?', 'What are the response attribution classes?', 'What are the speech acts?', 'What is the faithful response to this?', 'What is the corresponding part of speech tagging result to this sentence?'] 
    ...
    # POS data 
    with open(data_path[:-5] + '_pos' + data_path[-5:], 'r') as f3: 
        data_pos = json.load(f3) 

    for pair in data_pos: 
        questions.append(f'question: {question_list[4]} sentence: {pair["sentence"]}') 
        answers.append(pair["POS"]) 
